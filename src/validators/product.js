...
const editProductValidator = yup.object().shape({
    ...
    cover: yup.mixed()
        .test(
            'fileFormat',
            'فرمت کاور محصول معتبر نیست',
            value => {
                // اگر مقدار خالی باشد، اعتبارسنجی را عبور دهد
        ...
        .test(
            'fileSize',
            'حجم کاور محصول نمی‌تواند بیشتر از 1 مگابایت باشد',
            value => {
                // اگر مقدار خالی باشد (یعنی کاور جدیدی انتخاب نشده باشد)، اعتبارسنجی را عبور دهد
                if (!value) return true;
                return value.size <= 1024 * 1024; // 1 مگابایت
            }
        ),
